...
public class VerifiCode {
	...
	private String msgCodes = "0123456789";
	...
	private char randomMsgChar() // 获取随机字符
	{
		int index = r.nextInt(msgCodes.length());
		return msgCodes.charAt(index);
	}

	public String getMsgCode() {
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < 6; i++) // 画6个字符即可
		{
			char s = randomMsgChar();
			sb.append(s);
		}
		return sb.toString();
	}
	...
	private BufferedImage createImage() // 创建图片的方法
	{
		BufferedImage image = new BufferedImage(weight, height,
				BufferedImage.TYPE_INT_RGB); // 创建图片缓冲区
	...
	public void output(BufferedImage image, OutputStream out)
			throws IOException // 将验证码图片写出的方法
	{
